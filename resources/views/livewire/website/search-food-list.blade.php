      ...
      <div class="row">
        <div class="col-sm-12 col-md-8 col-lg-9">
          <div class="sorting-options d-flex flex-wrap justify-content-between align-items-center mb-30">
            <span><h2><strog>{{ __('message.We found') }} {{ $totalCount ?? '0' }} {{ __('message.results for') }} "{{$search ?? ''}}"</strog></h2></span>
            {{-- <select>
            ...
            @forelse($filterdata as $foodrow)
                ...
                <div class="product-item">
                    ...
                    <div class="product-info">
                      <p class="product-title"><a href="/GBooking/restaurant/foods/{{ base64_encode($foodrow->restaurant_id) }}" wire:navigate>{{ !empty($foodrow->restaurantName) ? ucwords($foodrow->restaurantName) : '' }}</a></p>
                    <h4 class="product-title"><a href="#">{{ !empty($foodrow->translationValue->food_translation_name) ? ucwords($foodrow->translationValue->food_translation_name) : '' }}</a></h4>
            ...
            <div class="widget widget-categories-layout2">
              ...
              <div class="widget-content">
                <ul class="list-unstyled mb-0">
                    @forelse($calegoryList as $catrow)
                     <li><a href="/GBooking/restaurant/foods/{{ base64_encode($catrow->restaurant_id) }}/{{ base64_encode($catrow->id) }}" wire:navigate>{{ !empty($catrow->translationValue->cat_translation_name) ? ucwords($catrow->translationValue->cat_translation_name) : '' }}</a></li>
                    @empty
